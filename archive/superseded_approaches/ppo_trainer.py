...
try:
    ...
    from .trajectory import (
        TrajectoryBuffer, collect_trajectory, collect_continuous_trajectory,
        _expand_graph_for_basis,
    )
...
class ContinuousPPOTrainer:
    ...
    def compute_ppo_loss_continuous(
        ...
        n_nodes_local = states.shape[1]
        n_minib_basis = states.shape[2]
        continuous_dim = states.shape[3]

        # Create expanded graph for batched basis processing
        (exp_ei, exp_ea, exp_ngi,
         exp_nf, exp_ng) = _expand_graph_for_basis(
            edge_index, edge_attr, node_graph_idx, node_features,
            n_nodes_local, n_graphs, n_minib_basis,
        )
        ...
        for mb_t in range(n_minib_steps):
            # Batch all basis states for this timestep
            t_idx = time_indices[mb_t, 0].item()

            # Flatten basis into batch: (n_nodes, n_basis, dim) -> (n_basis*n_nodes, dim)
            X_t_flat = states[mb_t].transpose(0, 1).reshape(-1, continuous_dim)
            X_next_flat = actions[mb_t].transpose(0, 1).reshape(-1, continuous_dim)
            rand_flat = rand_node_features[mb_t].transpose(0, 1).reshape(
                -1, rand_node_features.shape[-1]
            )

            # Single forward pass for all basis states at this timestep
            mean_flat, log_var_flat, values_flat, _ = model(
                X_t_flat, t_idx, exp_ei, exp_ea,
                exp_ngi, exp_ng, exp_nf,
                rand_nodes=rand_flat,
            )

            # New log probs for actions taken
            new_log_prob_flat = model.get_state_log_prob(
                mean_flat, log_var_flat, X_next_flat, exp_ngi, exp_ng
            )

            # Reshape: (n_basis * n_graphs,) -> (n_graphs, n_basis)
            new_state_log_prob = new_log_prob_flat.reshape(n_minib_basis, n_graphs).t()
            values_all = values_flat.reshape(n_minib_basis, n_graphs).t()

            old_state_log_prob = old_log_probs[mb_t]    # (n_graphs, n_basis)
            adv = advantages[mb_t]                       # (n_graphs, n_basis)
            v_target = value_targets[mb_t]               # (n_graphs, n_basis)

            # PPO ratio (vectorized over all graphs and basis states)
            ratios = torch.exp(new_state_log_prob - old_state_log_prob)
            all_ratios.append(ratios.reshape(-1))
            ...
            actor_loss = -torch.min(surr1, surr2).mean()
            critic_loss = F.mse_loss(values_all, v_target)

            total_actor_loss += actor_loss
            total_critic_loss += critic_loss

        n_total = n_minib_steps
        total_actor_loss /= n_total
        total_critic_loss /= n_total
    ...
    def train_step(
        ...
        with torch.no_grad():
            buffer = collect_continuous_trajectory(
                ...
                refine_noise_scale=refine_noise_scale,
                energy_norm=energy_norm,
            )

        # === Phase 2: Compute advantages ===
        # Use all basis states for reward statistics (no exclusion bias)
        reduced_rewards = buffer.rewards
        mov_mean, mov_std = self.moving_avg.update_mov_averages(reduced_rewards)
