...
def collect_continuous_trajectory(
    ...
    # Log prob of initial sample from N(0, I) prior
    # log N(x; 0, I) = -0.5 * (x^2 + log(2*pi)) per dim, summed over dims
    log_prior_per_dim = -0.5 * (X_prev ** 2 + np.log(2 * np.pi))
    log_prior_per_node = log_prior_per_dim.sum(dim=-1)  # (n_nodes, n_basis)
    log_q_T = torch.zeros(n_graphs, n_basis_states, device=device)
    for b in range(n_basis_states):
        log_q_T[:, b] = scatter_sum(
            log_prior_per_node[:, b], node_graph_idx, dim=0, dim_size=n_graphs
        )
    ...
    for step_idx in range(overall_diffusion_steps):
        ...
        step_state_log_probs = torch.zeros(n_graphs, n_basis_states, device=device)
        step_values = torch.zeros(n_graphs, n_basis_states, device=device)
        step_X_next = torch.zeros(
            n_nodes, n_basis_states, continuous_dim, dtype=torch.float32, device=device
        )
        step_rand_nodes = torch.zeros(
            n_nodes, n_basis_states, model.n_random_features, device=device
        )

        for b in range(n_basis_states):
            X_t_b = X_prev[:, b, :]  # (n_nodes, continuous_dim)
            ...
            # Sample action
            X_next_b, per_node_log_prob = model.sample_action(mean, log_var)

            # State log prob (summed per graph)
            state_log_probs_b = model.get_state_log_prob(
                mean, log_var, X_next_b, node_graph_idx, n_graphs
            )
        ...
        # Noise reward (Gaussian forward log prob)
        for b in range(n_basis_states):
            noise_reward_b = noise_schedule.compute_noise_reward(
                X_prev[:, b, :], step_X_next[:, b, :], model_step_idx,
                T_temperature, node_graph_idx, n_graphs
            )
            noise_rewards[step_idx, :, b] = noise_reward_b
        ...
        if per_step_energy and energy_fn is not None:
            for b in range(n_basis_states):
                energy_per_graph = energy_fn(
                    step_X_next[:, b, :], node_graph_idx, n_graphs
                )
    ...
    with torch.no_grad():
        for step_i in range(overall_diffusion_steps):
            ...
            for b in range(n_basis_states):
                ...
                mean, log_var, _, _ = model(
                    X_t_b, model_step_idx, edge_index, edge_attr,
                    node_graph_idx, n_graphs, node_features,
                )

                if T_temperature > 0:
                    X_next_b, _ = model.sample_action(mean, log_var)
                else:
                    X_next_b = mean  # Greedy: use mean directly
